...
export async function findUserByUsername(username: string): Promise<UserPrincipal | null> {
  const result = await pool.query<UserPrincipal>(
    `SELECT subject_id, tenant_id, username, display_name, enrolled, created_at::text
     FROM users
     WHERE username = $1`,
    [username]
  );
...
export async function createUser(username: string, displayName: string, tenantId = "default"): Promise<UserPrincipal> {
  ...
  const result = await pool.query<UserPrincipal>(
    `INSERT INTO users (subject_id, tenant_id, username, display_name, enrolled)
     VALUES ($1, $2, $3, $4, FALSE)
     RETURNING subject_id, tenant_id, username, display_name, enrolled, created_at::text`,
