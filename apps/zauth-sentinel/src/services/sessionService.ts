...
import type { Assurance, SentinelSession, SentinelUser } from "../types/models.js";
import { sha256Hex } from "../utils/security.js";
import { randomToken } from "../utils/security.js";

const SESSION_COOKIE = "sentinel_sid";
const OAUTH_STATE_COOKIE = "sentinel_oauth_state";
const PKCE_COOKIE = "sentinel_pkce_verifier";

export async function createSession(user: SentinelUser, assurance: Assurance): Promise<string> {
  const sessionId = randomToken(24);
  await pool.query(
    `INSERT INTO sentinel_sessions (session_id, user_id, subject_id, acr, amr, uid, did, expires_at)
...
export async function getSession(sessionId: string | undefined): Promise<SentinelSession | null> {
  if (!sessionId) {
    return null;
  }

  const result = await pool.query<SentinelSession>(
...
export async function deleteSession(sessionId: string | undefined): Promise<void> {
  ...
  await pool.query(`DELETE FROM sentinel_sessions WHERE session_id = $1`, [sessionId]);
}
...
export function createCsrfToken(session: SentinelSession): string {
  return sha256Hex(`${config.csrfSecret}:${session.session_id}:${session.subject_id}`);
}

export function validateCsrfToken(session: SentinelSession, token: string | undefined): boolean {
  if (!token) {
    return false;
  }
